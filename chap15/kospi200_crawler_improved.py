...
def crawl_kospi200_top_stocks():
    ...
    try:
        ...
        # 편입종목상위 섹션 찾기
        # h4 태그에서 "편입종목상위" 텍스트를 찾아서 해당 섹션을 찾습니다
        top_stocks_section = None
        
        # 방법 1: h4 태그로 찾기
        h4_tags = soup.find_all('h4', class_='top_tlt')
        for h4 in h4_tags:
            if '편입종목' in h4.get_text():
                # h4의 부모 div를 찾기
                parent_div = h4.find_parent('div', class_='box_type_m')
                if parent_div:
                    top_stocks_section = parent_div
                    break
        
        # 방법 2: 직접 box_type_m 클래스로 찾기
        if not top_stocks_section:
            box_divs = soup.find_all('div', class_='box_type_m')
            for div in box_divs:
                h4_in_div = div.find('h4', class_='top_tlt')
                if h4_in_div and '편입종목' in h4_in_div.get_text():
                    top_stocks_section = div
                    break
        ...
        print("편입종목상위 섹션을 찾았습니다!")
        
        # type_1 클래스를 가진 테이블 찾기
        table = top_stocks_section.find('table', class_='type_1')
        if not table:
...
def main():
    ...
    """
    print("=" * 60)
    print("네이버 금융 코스피200 편입종목상위 크롤러 (개선版)")
    print("=" * 60)
    
    print("\n1. 첫 페이지 편입종목상위 정보만 크롤링")
    print("2. 모든 페이지의 편입종목 정보 크롤링")
    
    choice = input("\n선택하세요 (1 또는 2): ").strip()
    
    if choice == "1":
        print("\n첫 페이지 편입종목상위 정보를 크롤링합니다...")
